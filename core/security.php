<?php
/**
 * Central security bootstrap for A-Code Playground.
 *
 ...
 *
 *   - acodeplayground_start_session()   secure, hardened session_start()
 *   - acodeplayground_security_headers() baseline HTTP security headers
...
if (!defined('ACODEPLAYGROUND_SECURITY_LOADED')) {
 ...
 * any output is sent.
 */
function acodeplayground_start_session() {
...
function acodeplayground_security_headers() {
 ...
 */
if (!defined('ACODEPLAYGROUND_ROOT')) {
    ...
    define('ACODEPLAYGROUND_ROOT', dirname(__DIR__));
}
function acodeplayground_throttle_file() {
    $dir = ACODEPLAYGROUND_ROOT . '/data/.security';
    if (!is_dir($dir)) { @mkdir($dir, 0775, true); }
    return $dir . '/login-attempts.json';
}

function acodeplayground_throttle_read($fh) {
    rewind($fh);
...
function login_is_locked($email) {
    ...
    if ($key === '') { return false; }
    $file = acodeplayground_throttle_file();
    if (!is_file($file)) { return false; }
    $fh = fopen($file, 'c+');
    if (!$fh) { return false; }
    flock($fh, LOCK_SH);
    $data = acodeplayground_throttle_read($fh);
    flock($fh, LOCK_UN);
    fclose($fh);
...
function login_register_failure($email) {
    $key = strtolower(trim((string)$email));
    if ($key === '') { return; }
    $fh = fopen(acodeplayground_throttle_file(), 'c+');
    if (!$fh) { return; }
    flock($fh, LOCK_EX);
    $data = acodeplayground_throttle_read($fh);
...
} // ACODEPLAYGROUND_SECURITY_LOADED
